...
interface MapSelectorProps {
  onAreaSelected?: (coordinates: number[][]) => void;
  onRunAlgorithm?: (area: number[][], officerCount: number, strategicZones?: StrategicZone[], calculatedPositions?: any[]) => void;
  requiredOfficers?: number;
}
...
const MapSelector: React.FC<MapSelectorProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Initialize map centered on Israel
const map = L.map(mapRef.current).setView([32.0724, 34.7767], 16);
    mapInstanceRef.current = map;
  ...
  // ✨ השינוי הקריטי - חישוב מיקומי שוטרים עם אזורים אסטרטגיים
  const calculateOfficerLocations = async () => {
    if (!isFirstUploadDone) {
      toast.warning('יש להעלות קובץ OSM ולוודא שהגרף קשיר לפני חישוב מיקומי השוטרים');
      return;
    ...
    try {
      ...
      // 🆕 שליחת בקשה עם אזורים אסטרטגיים
      const requestBody = {
        k: officerCount,
      ...
      console.log('🚀 שולח בקשת פיזור עם אזורים אסטרטגיים:', requestBody);

      const response = await fetch(`${serverBaseUrl}/api/KCenter/distribute-with-strategic`, {
        method: 'POST',
      ...
      const result = await response.json();
      console.log('✅ תוצאת פיזור:', result);

      const centers = result.PolicePositions || result.policePositions || [];

      if (centers.length > 0) {
        toast.info(`מקבל מיקומי קואורדינטות עבור ${centers.length} שוטרים...`);
        
        // המרת נתונים לפורמט המוכר
        const policeData = centers.map((center: any) => ({
          nodeId: center.NodeId || center.nodeId,
          lat: center.Latitude || center.latitude,
          lon: center.Longitude || center.longitude,
          isStrategic: center.IsStrategic || false
        }));
        ...
        if (mapInstanceRef.current) {
          ...
          policeData.forEach((officer, index) => {
            // בחירת אייקון לפי סוג השוטר
            const icon = officer.isStrategic ? strategicIcon : policeIcon;
            const officerType = officer.isStrategic ? 'שוטר אסטרטגי' : 'שוטר רגיל';
            
            // הוספת סימן שוטר
            const marker = L.marker([officer.lat, officer.lon], { icon })
              .addTo(map)
              .bindPopup(`${officerType} ${index + 1}<br/>מיקום: ${officer.lat.toFixed(5)}, ${officer.lon.toFixed(5)}<br/>מזהה צומת: ${officer.nodeId}`);
            ...
            if (result.MaxResponseTimeInSeconds || result.maxResponseTimeInSeconds) {
              ...
              const circle = L.circle([officer.lat, officer.lon], {
                radius: responseTime * 13.89,
                color: officer.isStrategic ? '#ff0000' : '#0062ff',
                fillColor: officer.isStrategic ? '#ff0000' : '#0062ff',
                fillOpacity: 0.1,
                weight: 1
              }).addTo(map);
        ...
        const strategicCount = result.StrategicOfficers || policeData.filter(p => p.isStrategic).length;
        const regularCount = result.RegularOfficers || policeData.filter(p => !p.isStrategic).length;

        toast.success(
          strategicCount > 0 
            ? `הוצגו ${policeData.length} שוטרים - ${strategicCount} אסטרטגיים ו-${regularCount} רגילים`
            : `הוצגו ${policeData.length} מיקומים אופטימליים לשוטרים`
        );
        
        // 🆕 העברת המידע המלא לקומפוננט האב
        onRunAlgorithm?.(
          selectedBounds ? [
            [selectedBounds.getNorth(), selectedBounds.getWest()],
            [selectedBounds.getNorth(), selectedBounds.getEast()],
          ...
          officerCount, 
          strategicZones,
          policeData  // 🎯 נתוני הפיזור המחושבים
        );
        ...
        <CardContent>
              ...
              <Card className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {isFirstUploadDone ? 'מוכן לפיזור' : 'נדרש העלאת אזור'}
                    </p>
                  </div>
                </div>
              </Card>
            ...
            <Card className="mt-4 p-4 bg-green-50 border-green-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-green-700">
                    <strong>אזורים אסטרטגיים:</strong> {policeLocations.filter(p => p.isStrategic).length}
                  </p>
                  <p className="text-sm text-green-700">
                    <strong>שוטרים רגילים:</strong> {policeLocations.filter(p => !p.isStrategic).length}
                  </p>
                </div>
                {maxResponseTime && (
                  <div>
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <ol className="text-sm text-command-slate-600 space-y-1">
              ...
              <li>2. לחץ על המפה במיקום הפינה הראשונה של הריבוע</li>
              <li>3. לחץ על המפה במיקום הפינה הנגדית לסיום הציור</li>
              <li>4. לחץ על "שליחת אזור לשרת" כדי לעבד את האזור</li>
              <li>5. (אופציונלי) לחץ על "בחירת אזורים אסטרטגיים" ובחר נקודות חשובות על המפה</li>
              <li>6. לחץ על "פיזור שוטרים" כדי לראות את הפיזור האופטימלי</li>
            </ol>
          </div>
        </CardContent>
      </Card>
      ...
      {policeLocations.length > 0 && (
        <Card className="command-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users className="w-5 h-5 text-command-indigo" />
              תוצאות הפיזור - אלגוריתם K-Center עם אזורים אסטרטגיים
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {policeLocations.map((officer, index) => (
                <div key={officer.nodeId || index} className={`p-3 rounded-lg border ${
                  officer.isStrategic 
                    ? 'bg-red-50 border-red-200' 
                    : 'bg-indigo-50 border-indigo-200'
                }`}>
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-command-slate-800">
                      {officer.isStrategic ? '🎯 שוטר אסטרטגי' : '👮 שוטר רגיל'} {index + 1}
                    </span>
                    <Badge variant="outline" className={
                      officer.isStrategic 
                        ? 'text-red-700 border-red-300' 
                        : 'text-command-indigo border-indigo-300'
                    }>
                      צומת #{officer.nodeId}
                    </Badge>
                  </div>
            ...
            {maxResponseTime && (
              <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200 text-center">
                ...
                <p className="text-xs text-green-600 mt-1">
                  הפיזור מבטיח כיסוי אופטימלי של כל האזור עם דגש על אזורים אסטרטגיים
                </p>
              </div>
            )}
          </CardContent>
