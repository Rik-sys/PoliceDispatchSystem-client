...
const CreateCallMap: React.FC<CreateCallMapProps> = ({ onLocationSelect }) => {
  ...
  return (
    <div className="relative w-full h-full">
      {/* Event selector */}
      <div className="absolute top-4 left-4 z-[1000] bg-white/90 backdrop-blur-sm rounded-lg p-3 shadow-lg min-w-[300px]">
        <Label htmlFor="event-select" className="text-sm font-medium text-slate-700">
          בחר אירוע
        </Label>
        <Select value={selectedEventId} onValueChange={handleEventSelect}>
          ...
          {/* כאן התיקון העיקרי - הוספת גובה מקסימלי וגלילה */}
          <SelectContent 
            className="bg-white border shadow-lg z-[2000] max-h-80 overflow-y-auto"
            position="popper"
            sideOffset={4}
          >
            {events.map((event) => (
              ...
              >
                <div className="flex flex-col py-1">
                  <span className="font-medium text-right">{event.eventName}</span>
                  <span className="text-xs text-slate-500 text-right mt-1">
                    {event.description}
                  </span>
                  <div className="flex justify-between items-center mt-1 text-xs text-slate-400">
                    <span>דחיפות: {event.priority}</span>
                    <span>שוטרים: {event.requiredOfficers}</span>
                  </div>
                </div>
              </SelectItem>
            ))}
