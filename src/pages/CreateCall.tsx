...
// const CreateCall = () => {
//   const navigate = useNavigate();
//   const [selectedLocation, setSelectedLocation] = useState<{lat: number, lng: number, eventId?: number} | null>(null);
//   const [formData, setFormData] = useState({
//     description: '',
//     priority: '',
//     eventId: '',
...
//   const handleLocationSelect = (lat: number, lng: number, eventId?: number) => {
//     setSelectedLocation({ lat, lng, eventId });
//     toast.success(`מיקום נבחר: ${lat.toFixed(5)}, ${lng.toFixed(5)}`);
//   };
...
//         EventId: selectedLocation.eventId, // Use the selected event ID
//         RequiredOfficers: formData.requiredOfficers,
//         Latitude: selectedLocation.lat,
//         Longitude: selectedLocation.lng,
...
//               <CardTitle className="flex items-center gap-2">
//                 <MapPin className="w-5 h-5 text-blue-600" />
//                 בחירת אירוע ומיקום הקריאה
//               </CardTitle>
//               <p className="text-sm text-slate-600">
//                 בחרו אירוע ולאחר מכן לחצו על המפה בתוך האזור האדום
//               </p>
//             </CardHeader>
//             <CardContent className="p-0">
//               <div className="h-96 relative rounded-b-lg overflow-hidden">
...
//                             אירוע ID: {selectedLocation.eventId}
//                           </div>
//                         )}
//                       </div>
//                     </div>
...
//                   <Button
//                     type="submit"
//                     disabled={isSubmitting || !selectedLocation || !selectedLocation.eventId}
//                     className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700"
//                   >
//                     {isSubmitting ? (
//                       <div className="flex items-center gap-2">
...
const CreateCall = () => {
  ...
  // הגדרת כתובת השרת - שנה את זה לכתובת השרת שלך
  const API_BASE_URL = 'https://localhost:7163'; // או הכתובת שלך
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // יצירת אובייקט הנתונים לפי הDTO של השרת
      const callDto = {
        eventId: selectedLocation.eventId,
        requiredOfficers: formData.requiredOfficers,
        contactPhone: formData.callerPhone || 'לא צוין',
        urgencyLevel: parseInt(formData.priority),
        callTime: new Date(formData.callTime).toISOString(),
        status: 'Open',
        latitude: selectedLocation.lat,
        longitude: selectedLocation.lng,
        // הוספת שדה Description אם השרת מצפה לו
       // Description: formData.description,
       // CallerName: formData.callerName || 'לא צוין'
      };



      console.log('שולח נתונים לשרת:', callDto);
      ...
      if (response.ok) {
        ...
        toast.error(`שגיאה ביצירת הקריאה: ${errorText || 'שגיאה לא ידועה'}`);
      }
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        toast.error('שגיאה בחיבור לשרת - בדוק את כתובת השרת');
      } else {
        toast.error(`שגיאה ביצירת הקריאה: ${error.message || 'שגיאה לא ידועה'}`);
      }
    } finally {
      setIsSubmitting(false);
    }
                    ...
                    <div>
                      ...
                      <Select value={formData.priority} onValueChange={(value) => setFormData({...formData, priority: value})}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="3">גבוהה</SelectItem>
                          <SelectItem value="4">קריטית</SelectItem>
                          <SelectItem value="5">חירום</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
